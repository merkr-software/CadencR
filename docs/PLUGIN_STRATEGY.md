...
Status: proposal with the local step-2 code-backed provider substrate implemented (rev. 2026-08-12). Grounded in a full audit of the service, desktop, and packaging layers, plus prior-art research (VS Code, Obsidian, Zed, JetBrains, Raycast, Figma) current to mid-2026.
...
- **2b — Code-backed agents, backend substrate. _Implemented in the service._** One generic ACP adapter (`providers/installed/`) consumes registry-shaped identity plus a Cadencr-owned installation record, but the referenced package executable owns provider-native discovery and mapping code. It must implement `models --format acp-config-options-v1` and `run --protocol acp-v1`; providers without a verified non-empty model list/default remain unavailable. Descriptors live in `<settings-dir>/providers/*.json` and are read once at startup; downloads and hot reload are later increments. Built-ins register first, so a descriptor cannot shadow one. Do **not** add descriptor capability booleans, model/mode lists, permission maps, or auth behavior.
- **2b.1 — Close the local-agent contract. _Backend contract, SDK, and Pi reference mapping implemented; arbitrary-executable UI withdrawn._** Strict/local descriptor profiles, stable diagnostics, provider origin, generic select/boolean session configuration, rich ACP v1 fixture coverage, and restart-gated lifecycle APIs are implemented. `cadencr-provider-sdk` supplies the versioned command harness, while `cadencr-pi-provider` maps Pi RPC models to the exact IDs used by `pi-acp`. Before the first prompt, Cadencr reconciles the pre-session model against the live ACP selector and requires authoritative confirmation. A simple UI for entering an executable, arguments, and environment is explicitly not the product contract: public installation waits for signed code-and-assets packages, conformance, integrity, and process policy. Durable resume, ACP v2, package distribution, negotiated-capability summaries, category-specific placement, and migration of legacy built-in controls remain open.
- **2b.2 — Developer authoring workspace. _Implemented; local-only and restart-gated._** Settings exposes **Add provider** as a project generator, not an executable installer. It creates a normal Git-backed user project and conversation without pane/worktree overrides, seeds `README.md` plus an agent-facing `INSTRUCTION.md`, and installs a local descriptor targeting the project's stable `bin/provider` output and connector-owned root `icon.svg`. The service validates and inlines that bounded package asset into the catalog, so adding branding never requires a Cadencr source edit. The developer uses their ordinary configured agent to implement the code-backed contract, merges/builds through their normal workflow, and restarts Cadencr between changes. Marketplace browsing and third-party package installation remain blocked on 2c plus M1/M2 safety.
