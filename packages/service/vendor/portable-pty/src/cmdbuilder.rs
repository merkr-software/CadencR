...
impl CommandBuilder {
    ...
    /// Override the value of `argv[0]` passed to the spawned process.
    ///
    /// This is useful for starting a shell as a login shell. A login shell
    /// is identified by its `argv[0]` starting with `-`. By setting
    /// `argv[0]` to `-sh` (or `-zsh`, etc.) the spawned shell will
    /// source user startup files.
    ///
    /// When `arg0` is set, it takes precedence over `args[0]` for the
    /// process's program name, while `args[0]` is still used to locate
    ...
    /// Returns the overridden `argv[0]`, if one was set via `arg0()`.
    #[cfg(unix)]
    pub fn get_arg0(&self) -> Option<&OsString> {
        self.arg0.as_ref()
    }
...
impl CommandBuilder {
    ...
    pub(crate) fn as_command(&self) -> anyhow::Result<std::process::Command> {
        ...
        let mut cmd = if self.is_default_prog() {
            ...
            // Use the custom argv[0] if set (e.g. "-sh" for login shells),
            // otherwise fall back to args[0].
            let argv0 = self.arg0.as_ref().unwrap_or(&self.args[0]);
            cmd.arg0(argv0);
